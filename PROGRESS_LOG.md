...
> **Note — `named_entity` now implemented (2026-05-29):** Added `inject_named_entity` and `_NE_SWAPS` to `injector.py`, and wired it into `build_eval_set`. Swap pairs cover company names (Client↔WDP), Swedish cities (Stockholm↔Gothenburg, Norrköping↔Jönköping), market segment (Large Cap↔Mid Cap), stock exchange, and certification standards. Will appear in results from next baseline run.
